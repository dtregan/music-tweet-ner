'''
File: models_track.py
Overall purpose: Train, run and evaluate the supervised models using the transformed 'TRACK' entities dataset.
...
import sys
f = open("model_track_output.txt", 'w')
sys.stdout = f
...
df = pd.read_csv('data_for_models_track.csv', encoding = "ISO-8859-1")
...
print(multilabel_confusion_matrix(y_test, y_pred=nb.predict(X_test), labels=classes))
print()
